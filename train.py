...
def train_one_epoch(model, dataloader, optimizer, scheduler, device, warmup_steps, warmup_scheduler):
    ...
    for batch_idx, (binned_expression,expression, gene_ids) in enumerate(progress_bar):
        ...
        # 常规精度前向传播
        recon, cls_pred, mask,_ = model(binned_expression, gene_ids)
        recon_loss = model.loss_function(expression, recon, mask)
        
        # 添加分类损失（如果适用）
        cls_loss = 0.0  # 这里假设分类任务未实现，但保留结构
...
def save_checkpoint(epoch, model, optimizer, scheduler, warmup_scheduler, best_loss, best_epoch, is_best, model_output_dir):
    """保存检查点文件，包含最佳损失对应的epoch"""
    state = {
        'epoch': epoch + 1,  # 保存下一个epoch作为开始
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'scheduler_state_dict': scheduler.state_dict(),
        'warmup_scheduler_state_dict': warmup_scheduler.state_dict(),
        'best_loss': best_loss,
        'best_epoch': best_epoch  # 新增：保存最佳epoch
    }
...
def load_checkpoint(model, optimizer, scheduler, warmup_scheduler, model_output_dir):
    ...
    if os.path.exists(checkpoint_path):
        ...
        best_epoch = checkpoint.get('best_epoch', -1)  # 恢复最佳epoch
        
        return start_epoch, best_loss, best_epoch, True
    return 0, float('inf'), -1, False  # 从头开始训练
...
def train(config, model, dataloader, optimizer, scheduler, device, warmup_steps, warmup_scheduler, model_output_dir):
    """训练主函数（支持断点续训），记录最佳epoch和每个epoch的损失"""
    # 加载检查点
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        print(f"记录第 {epoch+1} 个epoch的损失: {epoch_loss:.8f}")
        ...
        if torch.cuda.is_available():
            ...
            print(f"GPU内存使用: {gpu_mem:.1f} GB | "
                  f"检查点保存至: {checkpoint_path}")

    # 训练完成后输出最佳结果
    if best_epoch > 0:
        print(f"训练完成! 最佳损失: {best_loss:.8f} (出现于第 {best_epoch} 个epoch)")
    else:
        print(f"训练完成! 最终损失: {best_loss:.8f} (未观察到改进)")
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'batch_size': 2,     # 批次大小
        'num_workers': 63,     # 数据加载工作进程数
        'epochs': 200,        # 训练轮数
        'data_dir': "project1/spatial_data/samples22", # 数据目录
        'pad_id': vocab["<pad>"] if vocab else 0,  # 填充ID
        'num_genes': max(vocab.values()) + 1 if vocab else 1000, # 基因数量 (包括pad)
        'model_output_dir': 'project1/model_outputs/maskdual_valid512(22)', # 模型输出目录
        'model_type': 'transformer', # 模型类型
        'tips': '基因id和表达值分别embedding'
    }
