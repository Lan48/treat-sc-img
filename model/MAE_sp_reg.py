...
class MAEModel(nn.Module):
    ...
    def _create_spatial_mask(self, original):
        """
        根据 original 张量 (B, C, H, W) 动态生成空间有效性掩码 (B, 1, H, W)。
        一个 Spot 只有当其所有通道的值都不为零时，才被视为有效。
        """
        # 1. 在通道维度 C 上求和 (B, H, W)
        # 如果一个 Spot (h, w) 是有效的，其所有通道的基因值之和应大于零。
        # 考虑到浮点数精度，使用容忍度判断
        sum_channels = torch.sum(original.abs(), dim=1, keepdim=True) # 形状 (B, 1, H, W)
        
        # 2. 生成二值掩码
        # 如果和 > zero_tolerance，则该位置有效 (1)，否则无效 (0)
        spatial_mask = (sum_channels > 1e-6).float()
        
        return spatial_mask
    def _get_diff_kernels(self, channels, device):
        # 垂直差分核 (中心-下方)
        kernel_v = torch.FloatTensor([[0, 0, 0], [0, 1, 0], [0, -1, 0]]) 
        # 水平差分核 (中心-右方)
        kernel_h = torch.FloatTensor([[0, 0, 0], [0, 1, -1], [0, 0, 0]]) 
        
        # 扩展到 Group Conv 格式 (C, 1, 3, 3)
        kernel_v = kernel_v.view(1, 1, 3, 3).repeat(channels, 1, 1, 1).to(device)
        kernel_h = kernel_h.view(1, 1, 3, 3).repeat(channels, 1, 1, 1).to(device)
        return kernel_v, kernel_h

    def _R_sp_regularization(self, reconstructed, spatial_mask):
        """
        计算空间平滑正则项 R_sp (Eq. 17)，考虑零填充掩码。
        reconstructed: 形状 (B, C, H, W)。
        spatial_mask: 形状 (B, 1, H, W)。1=有效Spot, 0=零填充。
        """
        B, C, H, W = reconstructed.shape
        device = reconstructed.device
        
        # 确保 spatial_mask 具有正确的 float 类型和设备
        spatial_mask_float = spatial_mask.float().to(device)
        
        kernel_v, kernel_h = self._get_diff_kernels(C, device)

        # 1. 计算差分 (Group Conv)
        diff_v = F.conv2d(reconstructed, weight=kernel_v, padding=1, groups=C)
        diff_h = F.conv2d(reconstructed, weight=kernel_h, padding=1, groups=C)

        # 2. 计算有效性掩码 (Validity Mask)
        # 两个 Spot 都有效 (值 >= 2.0)
        # 掩码卷积使用 groups=1
        mask_kernel_v = torch.FloatTensor([[0, 0, 0], [0, 1, 0], [0, -1, 0]]).view(1, 1, 3, 3).abs().to(device)
        mask_kernel_h = torch.FloatTensor([[0, 0, 0], [0, 1, -1], [0, 0, 0]]).view(1, 1, 3, 3).abs().to(device)
        mask_conv_v = F.conv2d(spatial_mask_float, weight=mask_kernel_v.clone().abs().to(device), padding=1, groups=1)
        mask_conv_h = F.conv2d(spatial_mask_float, weight=mask_kernel_h.clone().abs().to(device), padding=1, groups=1)
        
        valid_diff_mask_v = (mask_conv_v >= 2.0).float()
        valid_diff_mask_h = (mask_conv_h >= 2.0).float()

        # 3. 应用掩码并计算损失
        # masked_diff_v_sq 形状: (B, C, H, W)
        masked_diff_v_sq = (diff_v**2) * valid_diff_mask_v.repeat(1, C, 1, 1)
        masked_diff_h_sq = (diff_h**2) * valid_diff_mask_h.repeat(1, C, 1, 1)
        
        # 4. 聚合损失 (只对有效差值的数量求平均)
        num_valid_v = torch.sum(valid_diff_mask_v)
        num_valid_h = torch.sum(valid_diff_mask_h)
        
        R_sp = torch.tensor(0.0, device=device)
        if num_valid_v > 0 or num_valid_h > 0:
            # 公式中的 (1/C) 被隐含在最后对所有元素的平均中 (但我们只对有效元素平均)
            
            # 对所有有效差值求和
            sum_diff_v = torch.sum(masked_diff_v_sq)
            sum_diff_h = torch.sum(masked_diff_h_sq)
            
            # 使用有效差值的总数进行归一化
            # C 是通道数，即每个有效差值贡献 C 个项到总和中
            loss_v = sum_diff_v / (num_valid_v * C) if num_valid_v > 0 else 0
            loss_h = sum_diff_h / (num_valid_h * C) if num_valid_h > 0 else 0
            
            # 平均垂直和水平方向的损失
            R_sp = (loss_v + loss_h) / 2.0
    ...
    def loss_function(self, original, reconstructed, mask):
        ...
        """
        mask = mask.bool()
        spatial_mask = self._create_spatial_mask(original)
        loss = F.mse_loss(original[mask], reconstructed[mask])
        R_sp = self._R_sp_regularization(reconstructed, spatial_mask)
        L_sp = 0.001 * R_sp
        loss = loss + L_sp
        return loss
