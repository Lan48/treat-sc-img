...
def train_mlp_on_adata(adata, obsm_key, obs_label_key, test_size=0.2, 
                       ...
                       knn_k=43, predict_key='mlp_prediction', save_path=None):  # 新增predict_key和save_path参数
    """
...
def main():
    ...
    for train_file in train_files:
        ...
        # ---------- 关键：给 spot 名加前缀 ----------
        prefix = train_file.replace('.h5ad', '')  # 去掉后缀，干净一点
    ...
    if len(adata_train_list) == 1:
        ...
        adata_train = adata_train_list[0].concatenate(
            ...
            index_unique=None          # 此时 obs_names 已唯一，无需再后缀
        )
    #print(adata_train.obs['Pathologist Annotation'].unique())    
    # 随机划分训练集测试集
    model, X_test, y_test, history, le, scaler,feature_info = train_mlp_on_adata(
        adata_train, 
        obsm_key='X_emb512_model40_16_sp_reg_v2',  # 使用的特征 X_emb_scGPTspatial、X_emb_scGPT、X_emb512_model40_16_uniform_random
        obs_label_key='sce.layer_guess',  # 细胞类型标签 DLPFC:sce.layer_guess human_lymph_node:manual-anno
        test_size=0.8,                    # 细胞类型标签 Colorectal cancer:Pathologist Annotation human_breast_cancer\ST_Hippocampus:ground_truth
        epochs=800,                       # 细胞类型标签 Human_tonsil:final_annot
        warm_up_epochs=20,  # 使用warm up策略
        warm_up_factor=0.1, # 初始学习率为最终学习率的1%
        structure='mlp',  # 使用'mlp'或'transformer'
        knn_k=0,
        predict_key='X_emb512_model40_16_sp_reg_v2',# data_split 表示测试集训练集
        save_path='project1/spatial_data/down_stream_data/raw_data_DLPFC/k_with_emb_with_emb_spreg.h5ad',
    )
    new_adata = ad.read_h5ad("project1/spatial_data/down_stream_data/raw_data_DLPFC/k_with_emb_with_emb_spreg.h5ad")
